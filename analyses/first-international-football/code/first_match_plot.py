...
def make_label(row):
    ...
    return (
        f'<b>{row.team_name}</b><br>{row.ko_date}<br>v {row.oppname}'
        f'<br>{row.outcome} {row.score}'
    )
...
def main(args):
    ...
    for i, y in enumerate(years):
        ...
        title = dict(
            text=f'The Beautiful Game Goes Global: {y}',
            font={'size': 30},
            xref='paper',
            xanchor='left',
            x=0
        ...
        subtitle_text = (
            "Tracking the first official match for each international level men's team of FIFA Member states"
            " | Matches not necessarily against other full FIFA members<br>"
            "Darker green represents older team"
            " | Pink dots represent debut nations"
        )
        debutants_text = f'Debuts: {", ".join(dfdebs.team_name.unique())}'
        credits_text = 'Graph: @awgymer | Data: https://en.wikipedia.org & https://www.fifa.com/associations/'
